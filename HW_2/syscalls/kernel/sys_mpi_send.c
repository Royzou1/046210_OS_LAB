...
int sys_mpi_send(pid_t pid, char *message, ssize_t message_size)
{
    ...
    debug_print("*** ------------ into sending msg --------------- *** \n");
   if (DEBUG) { printk(KERN_ERR"the message is: %s , and it length is: %d\n", message, message_size);}
    ...
    if(sender == NULL)
    {
        if(DEBUG) printk(KERN_ERR "error in get task_struct pf sender \n");
        return -ESRCH;
    }
    ...
    //msg sent successuly wake up if waiting for me!
    int npids = receiver->poll_list_size;
    if (receiver->poll_list != NULL) {
        int i;
        for (i = 0 ; i < npids ; i++) {
            if (receiver->poll_list[i] == sender->pid) {
                wake_up_process(receiver);
            }
        }
    } 

    debug_print("*** ------------ leaving sending msg --------------- *** \n");
    return SUCCESS;
}